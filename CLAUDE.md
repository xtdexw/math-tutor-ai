...
- **v2.0.0** (2025-01-08): 升级到 Qwen3-VL 多模态模型，新增图片上传功能
- **v1.0.0** (2025-12-31): 初始版本，基于 DeepSeek-V3.2
...
### 项目定位
将抽象的数学定理与公式转化为直观易懂的语音讲解，通过3D数字人AI老师提供实时交互式数学学科辅导，支持文字对话和图片题目识别，协助学生攻克知识盲区，提供系统化的专项辅导。

### 核心理念
"新一代课程，不是视频，也不是书，而是一个会陪你学习的AI老师"——让AI拥有"老师的身体与对话能力"，把学习从"看内容"升级为一个能实时对话、能看图解题、能引导学习、能陪练的AI老师。
...
#### 1.1 自由对话模式 ✅
- **功能描述**：学生可以自由提问任何数学问题，或上传数学题目图片，AI老师实时解答
- **交互方式**：
  - 文字输入：在输入框中输入问题，按 Enter 发送
  - Shift+Enter：换行
  - 图片上传：点击"📷 上传"按钮选择本地图片
  - 图片链接：点击"🔗 链接"按钮输入图片 URL
- **用户体验**：
  - 实时流式响应显示
  - 打字指示器动画
  - 自动滚动到最新消息
  - 清空历史功能
  - 智能消息格式化（自动分段、公式高亮、列表识别）
- **状态流程**：
  ```
  用户输入（文字/图片）→ 倾听状态(listen) → 思考状态(think) → 流式讲解(speak) → 互动待机(interactive_idle)
  ```
...
- **配置项**：
  ...
  - ModelScope API Key 配置（Qwen3-VL）
- **特色功能**：
...
#### 1.5 多模态AI响应 ✅
- **功能描述**：基于 Qwen3-VL-235B-A22B-Instruct 的多模态对话能力
- **技术特性**：
  - 支持图片识别与分析（多模态）
  - 实时流式输出
  - 智能文本清理（移除 emoji、markdown 格式）
  - 对话历史管理（最多20条）
  - 智能消息格式化（MessageText组件）
- **语音优化**：
  - 缓冲机制，保证语音流畅
  - 定时输出策略
  - 只保留中文标点和适合语音的文本
...
#### AI能力 ✅
- **魔搭社区 Qwen3-VL-235B-A22B-Instruct**
...
#### 3.2 QwenVLService - 多模态AI模型服务

**文件位置**: `src/services/QwenVLService.js`

**核心职责**:
- 封装 Qwen3-VL-235B-A22B-Instruct API
- 支持图片识别与分析（多模态）
- 流式响应处理
- 文本清理
- 对话历史管理
...
**流式调用（支持图片）**:
```javascript
async chatStream(userMessage, imageUrl, onThinking, onContent, onDone, onError)
```

**回调说明**:
- `onThinking(reasoning_content)` - 思考过程回调（保留兼容，QwenVL不返回）
...
**核心职责**:
- 整合 AvatarService 和 QwenVLService
- 实现完整对话流程（支持图片）
- 管理语音输出缓冲

**核心方法**:

1. **handleUserInput(userInput, imageUrl, callbacks)**
   ```javascript
   // 处理用户输入并驱动数字人响应（支持图片）
   // 流程: listen → think → QwenVL → speak → interactiveIdle
   // 参数: userInput(用户输入), imageUrl(图片URL,可选), callbacks(回调函数)
   ```
...
**文件统计**:
- 源代码文件: 11个核心文件
- 组件: 6个 Vue 组件（含 MessageText.vue）
- 服务: 3个 JavaScript 服务
- 知识点: 16个预置知识点
...
```javascript
{
  ...
  modelscope: {
    apiKey: string           // 魔搭社区 API Key（Qwen3-VL）
  }
}
```
...
├──────────────────────────────┬─────────────────────────┤
│                              │                         │
│  Avatar Section (40%)        │   Interaction (60%)     │
│  (flex: 0 0 40%, 最小500px)  │   ├─ Tab Panel         │
│  ┌──────────────────────┐    │   │  ├─ 对话 Tab       │
│  │                      │    │   │  │  ├─ 消息列表    │
│  │   3D数字人渲染区      │    │   │  │  ├─ 输入框      │
│  │                      │    │   │  │  ├─ 图片上传    │
...
#### 1.3 多模态AI响应 ✅
- **实时流式输出**: 利用 Qwen3-VL 的流式能力
- **图片识别支持**: 多模态模型，支持图片题目识别与分析
- **智能文本清理**: 自动移除 emoji 和 markdown 格式
- **智能消息格式化**: MessageText 组件实现自动分段、公式高亮、列表识别
- **语音缓冲机制**: 保证语音输出的流畅性
...
#### 2.2 技术集成创新 🚀
- **SDK + 多模态LLM 深度集成**: 星云SDK与Qwen3-VL无缝对接
- **图片题目识别**: 支持上传数学题目图片，AI自动识别并讲解
- **智能消息格式化**: MessageText组件实现自动分段、公式高亮、列表识别
- **配置灵活管理**: 支持测试配置快速体验
...
- **系统集成**: 星云SDK + Qwen3-VL 深度集成
- **图片识别**: 支持上传数学题目图片，AI自动识别并讲解
- **智能格式化**: MessageText组件实现消息自动分段、公式高亮、列表识别
- **可扩展性**: 模块化设计，易于扩展新功能
...
- **Qwen3-VL模型**: https://modelscope.cn/models/Qwen/Qwen3-VL-235B-A22B-Instruct
...
1. **技术创新**: 首创多模态流式AI响应 + 3D数字人驱动的实时教育模式
2. **产品创新**: AI老师具身化 + 图片题目识别，提供沉浸式学习体验
3. **体验创新**: 状态机驱动的交互流程，实时反馈，智能消息格式化
...
**星云智教**项目充分展示了魔珐星云具身驱动SDK和Qwen3-VL多模态大模型的强大能力，打造了一个技术先进、功能完整、体验优秀的AI教育应用。项目创新性地实现了图片题目识别功能，让AI不仅能对话，还能"看题"讲解，极大地提升了用户体验和学习效果。项目具有较高的商业价值和广阔的市场前景，是一个成功的黑客松作品。
